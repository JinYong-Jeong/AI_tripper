...
import { Menu, X, ArrowLeft, MapPin, Award, LogOut } from 'lucide-react';
...
interface GusukTourDaejeonPageProps {
...
export default function GusukTourDaejeonPage({ onNavigate }: GusukTourDaejeonPageProps) {
  ...
  const menuItems: MenuItem[] = [
    ...
    { 
      ...
      title: '전국 스탬프', 
      type: 'stamp',
      action: () => onNavigate('national-stamp')
    },
    { 
      id: 'coupon', 
      title: '쿠폰 보관함', 
  ...
  const handleMenuItemClick = (item: MenuItem) => {
    if (item.action) {
      item.action();
      setIsMenuOpen(false);
    } else if (item.type === 'attraction' && !item.action) {
      // 미구현 관광지 알림
  ...
  return (
    <div className="min-h-screen relative">
      {/* AI 콕콕 트리퍼 메인으로 돌아가기 버튼 */}
      <div className="absolute top-6 right-6 z-50">
        <Button
          onClick={() => onNavigate('gusuk-tour-main')}
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6">
        ...
        >
          <h1 className="font-gmarket-bold text-4xl tracking-wide mb-4 text-gray-800">트리퍼 in 대전</h1>
          <p className="text-lg text-gray-600">대전의 숨겨진 명소와 특별한 경험을 함께 발견해보세요!</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl w-full">
              ...
              <div className="p-5 bg-gradient-to-b from-amber-50 to-orange-50">
                <div className="text-center">
                  ...
                  <p className="text-amber-700 text-sm leading-relaxed">
                    대전 주요 관광지의 실시간 정보와<br />
                    숨겨진 보물 같은 특별한 명소들을 발견해보세요!
                  </p>
                </div>
              </div>
          ...
          >
            <Card 
              ...
              onClick={() => onNavigate('daejeon-stamp')}
            >
              {/* 이미지 섹션 */}
              <div className="relative h-40 overflow-hidden">
